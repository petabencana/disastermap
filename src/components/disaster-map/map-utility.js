...
export class MapUtility {
    ...
    onLocationFound(e) {
        ...
        for (let city in regions) {
            ...
            if (
                e.latitude > regions[city].bounds.sw[0] &&
                e.longitude > regions[city].bounds.sw[1] &&
                e.latitude < regions[city].bounds.ne[0] &&
                e.longitude < regions[city].bounds.ne[1]
            ) {
    ...
    viewClientLocation(map, layers, reportsStatsMessage, togglePane) {
        ...
        if (self.clientLocation) {
            if (self.clientCityIsValid) {
                ...
                map.once("moveend", () => {
                    //execute only once, after fly to location ends, ignores user map drag events
